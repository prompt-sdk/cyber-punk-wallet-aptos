...
import { AgentData } from '../interfaces/dashboard.interface';
...
const DashboardWidget: FC<DashboardWidgetProps> = ({ className, session }) => {
  ...
  const createDefaultAgent = useCallback(async () => {
    ...
    console.log('🚀 ~ createDefaultAgent ~ randomAvatar:', randomAvatar);
  ...
  const createAgentAPI = async (agentData: AgentData) => {
    try {
  ...
  return (
    ...
    >
      ...
      {selectedAgent && (
        <ChatPopup visible={isOpenModal} inforAgent={selectedAgent} onClose={() => setIsOpenModal(false)} />
      )}
    </BoderImage>
  );
};
